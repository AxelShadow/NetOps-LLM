...
14. Сетевые устройства (eltex/mikrotik/usergate) и принтеры опрашивай по SNMP:
    ...
    snmp_walk (произвольный OID). Счётчики принтера — printer_info:
    распечатанные страницы, серийный номер, уровень тонера, статус и ошибки
    (замятие, нет бумаги); device='all' — все принтеры сразу. Счётчик
    сканов вендорозависим и стандартным Printer-MIB не отдаётся — если
    нужен, скажи об этом пользователю, не выдумывай значение."""
