...
### Этап 21 — RAG и планировщик задач (готово, 2026-09-21)

Реализованы два новых модуля для расширения функциональности системы:

**RAG (Retrieval-Augmented Generation):**
- Модуль `backend/app/rag/` с сервисом векторного поиска по документации
- ChromaDB в качестве векторного хранилища с персистентностью
- SentenceTransformers для генерации эмбеддингов (модель all-MiniLM-L6-v2 по умолчанию)
- Автоматическое определение кодировки файлов (chardet)
- Умное разбиение документов на чанки с перекрытием
- Поиск релевантных фрагментов по запросу с метаданными
- API для добавления документов, поиска и управления коллекцией
- Переменные окружения: `RAG_ENABLED`, `CHROMA_PERSIST_DIR`, `EMBEDDING_MODEL`

**APScheduler (Планировщик задач):**
- Модуль `backend/app/scheduler/` с асинхронным планировщиком
...
- Логирование выполнения задач в базу данных (TaskLog)
- Обработчики ошибок и успешного выполнения
- Graceful shutdown при остановке приложения
- Переменные окружения: `SCHEDULER_ENABLED`, `REPORT_RECIPIENTS`

**Обновления:**
- `backend/requirements.txt`: добавлены chromadb, sentence-transformers, chardet, apscheduler
- `.env.example`: добавлены переменные для RAG и планировщика
- Интеграция с основным приложением через `main.py` (требуется добавить инициализацию сервисов)

**Следующие шаги:**
- Интеграция RAG-поиска в LLM-агент (инструмент search_internal_docs)
- Настройка доставки отчетов в Telegram/почту
- Добавление UI для загрузки документов в базу знаний
- Расширение списка планируемых задач
...
- Прод на Ubuntu: docker compose, реальный AD (убрать DEV_MODE), TLS,
  alembic-миграции, сетевая сегментация (раздел 3).
